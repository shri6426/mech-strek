...
export default function ClientLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full p-8 rounded-2xl bg-[#111]/80 backdrop-blur-xl border border-[rgba(255,255,255,0.08)] shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8 text-center">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-blue-500/20 to-purple-500/20 border border-white/10 flex items-center justify-center mb-4">
            <Lock className="w-5 h-5 text-blue-400" />
          </div>
          <h1 className="text-2xl font-bold tracking-tight">Client Portal</h1>
          <p className="text-xs text-neutral-400 mt-1">Sign in to view your projects and invoices</p>
        </div>
        ...
        {/* Sample User Credentials Box */}
        <div className="mb-6 p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 space-y-2">
          <div className="font-semibold flex items-center justify-between">
            <span>🔑 Sample Client Test Account</span>
            <button
              type="button"
              onClick={() => { setEmail('client@techstart.in'); setPassword('ClientPassword123!'); }}
              className="text-[11px] font-semibold bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded-lg transition-all shadow-[0_0_10px_rgba(59,130,246,0.3)]"
            >
              Auto-Fill Test Account
            </button>
          </div>
          <div className="text-[11px] text-neutral-300 space-y-0.5 font-mono">
            <div>Email: <span className="text-white">client@techstart.in</span></div>
            <div>Client: <span className="text-neutral-400">Rahul Sharma (TechStart India)</span></div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-6 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-semibold py-3 rounded-xl text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-[0_0_20px_rgba(59,130,246,0.3)]"
          >
          ...
          <button
            ...
            onClick={() => {
              ...
              window.location.href = `${apiBase}/auth/google/login?state=client`;
            }}
