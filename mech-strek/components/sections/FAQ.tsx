...
import { fetchFaqItems, FaqItem as FaqItemType } from '@/lib/api';
...
function Item({ faq }: { faq: { q: string; a: string } }) {
    ...
    <section id="faq" className="py-28" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          >
            {faqList.map((f) => <Item key={f.q} faq={f} />)}
          </motion.div>
        </div>
      </div>
    </section>
